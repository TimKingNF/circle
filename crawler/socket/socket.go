...
type Socket interface {
	...
	//	socket parse data to scheduler chan
...
type mySocket struct {
	...
	parseChan  chan *cmn.ControlMessage
...
func (socket *mySocket) parseData() {
	go func() {
		for {
			...
			genLogger().Infoln(fmt.Sprintf("Received request[%s]: %s (crawler socket)\n",
...
func (socket *mySocket) Run() {
	...
	socket.parseChan = make(chan *cmn.ControlMessage)
...
func (socket *mySocket) Send(data string) {
	...
	genLogger().Infoln(fmt.Sprintf("Send Data[%s]: %s (crawler socket)\n",
